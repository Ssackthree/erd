...
CREATE TABLE delivery (
    ...
    CONSTRAINT FK_delivery_rule_TO_delivery FOREIGN KEY (delivery_rule_id) REFERENCES delivery_rule(delivery_rule_id),

    -- orders_id를 orders 테이블의 orders_id와 연결하는 외래 키
    CONSTRAINT FK_orders_TO_delivery FOREIGN KEY (orders_id) REFERENCES orders(orders_id)
);
...
CREATE TABLE discount (
    ...
    -- book_id를 book 테이블의 book_id와 연결하는 외래 키
    CONSTRAINT FK_book_TO_discount FOREIGN KEY (book_id) REFERENCES book(book_id)
);
